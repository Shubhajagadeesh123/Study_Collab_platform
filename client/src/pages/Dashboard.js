...
const Dashboard = ({ onLogout }) => {
  ...
  const handleFileUpload = (event) => {
    ...
    alert(`${file.name} added to your repository!`);
  };
      ...
      <div className="w-64 bg-white border-r border-gray-200 p-6 flex flex-col">
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between px-4 mb-4">
            ...
            <button onClick={handleFileClick} className="h-6 w-6 flex items-center justify-center rounded-full bg-indigo-600 text-white font-bold hover:bg-indigo-700 transition">+</button>
            <input type="file" ref={fileInputRef} onChange={handleFileUpload} className="hidden" accept=".pdf,.mp4" />
          </div>
          
          <div className="space-y-1 overflow-y-auto max-h-64 pr-2">
            {resources.map(res => (
              <div key={res.id} className="px-4 py-2.5 text-xs text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 cursor-pointer rounded-xl flex items-center transition-all">
                <span className="mr-3 opacity-70">{res.type === 'PDF' ? '📄' : '🎥'}</span>
        ...
        <div className="mt-auto p-4 bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-2xl text-white">
          <p className="text-[10px] font-bold uppercase opacity-80 mb-1">Premium Plan</p>
          <p className="text-xs font-medium mb-3">10GB cloud storage</p>
      ...
      <div className="flex-1 flex flex-col">
        <header className="h-20 bg-white border-b border-gray-200 px-8 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            />
            <button onClick={joinGroup} className="bg-indigo-600 text-white px-5 py-2.5 rounded-2xl text-sm font-bold hover:bg-indigo-700 transition shadow-md shadow-indigo-100">Join</button>
          </div>
        ...
        <main className="flex-1 overflow-hidden flex flex-col p-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-2xl font-black text-gray-900 tracking-tight">Virtual Study Room</h2>
          ...
          <div className="flex-1 bg-white rounded-[32px] border border-gray-100 shadow-xl shadow-gray-200/50 flex flex-col mb-4 overflow-hidden">
            ...
            <div className="p-6 bg-gray-50/50 backdrop-blur-sm flex items-center space-x-4">
              ...
              <button onClick={sendMessage} className="bg-indigo-600 p-4 rounded-2xl text-white hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all active:scale-90">
                <svg className="w-5 h-5 fill-current" viewBox="0 0 20 20"><path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z"/></svg>
              </button>
            </div>
          </div>
        </main>
